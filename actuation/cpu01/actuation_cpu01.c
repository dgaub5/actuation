    ...
    Uint16 ToggleCount = 0;         // Initialize the count toggle - not used (can delete?)
    Uint16 mmSpeed;                 // {-600, 600} [rad/s] - Motor Mechanical Speed rad/s | {0.0 V, 3.3 V} - not used (change name of AdcaResults array to this?)
    Uint16 maCurrent;               // {-2.5, 2.5} [A]     - Motor Armature Current in A  | {0.0 V, 3.3 V} - not used (change name of AdccResults array to this?)
    ...
    float32 AdcaResults[RESULTS_BUFFER_SIZE];   // Allocate memory for the ADC-A registers (motor speed) - maybe change name of array to mmSpeed (?)
    float32 AdccResults[RESULTS_BUFFER_SIZE];   // Allocate memory for the ADC-C registers (armature current) - maybe change name of array to maCurrent (?)
    ...
    void main(void)
    {
        ...
        {
            AdcaResults[resultsIndex] = 0;      // Set ADC-A current results index to 0
            AdccResults[resultsIndex] = 0;      // Set ADC-C current results index to 0
        }
    ...
    interrupt void adca1_isr(void)
    {
        ...
        if (trigger != 0)
        {
            AdcaResults[resultsIndex] = 0.293 * (AdcaResultRegs.ADCRESULT0-2048);       // Store current value of ADC-A in array, scaled to {-600 to 600} [rad/s]
            DutyCycle = AdcbResultRegs.ADCRESULT0;                                      // Update DutyCycle with ADC-B results
            AdccResults[resultsIndex++] = 0.00122 * (AdccResultRegs.ADCRESULT0-2048);   // Store current value of ADC-C in array, scaled to {-2.5 to 2.5} [A]
