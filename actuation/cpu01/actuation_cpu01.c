    ...
    int16 mmSpeed = 0x000;         // {-600, 600} [rad/s]  - Motor Mechanical Speed rad/s  | {0.0 V, 3.3 V}
    int16 maCurrent = 0x000;       // {-2.5, 2.5} [A]      - Motor Armature Current in A   | {0.0 V, 3.3 V}

    //Variables for input
    Uint16 dacOutput;
    volatile int16 LoadTorque = 0.1;   // {-0.2, 0.2}  [Nm]    - Load Torque in Nm | {0.0 V, 3.0 V}
    volatile Uint16 DutyCycle = 95;     // {0, 100}     [%]     - Duty Cycle in %   | {0.0 V, 3.0 V}
    volatile bool interrupt_flag = 0;   // set interrupt flag initially to 0
    ...
    interrupt void adca1_isr(void);     // ADC interrupt service routine for storing mmSpeed and maCurrent
    interrupt void adcb1_isr(void);      // ADC interrupt service routine for updating DutyCycle and LoadTorque values from GUI
    ...
    int16 AdccResults[RESULTS_BUFFER_SIZE];
    Uint16 resultsIndex;
    Uint16 pretrig = 0;
    Uint16 trigger = 0;
    ...
    void main(void)
    {
        ...
        PieVectTable.ADCA1_INT = &adcb1_isr;            // Function for ADCB interrupt 1
        EDIS;
        ...
        DutyCycle   = (DutyCycle/100)*4095;             // convert Duty Cycle to bit representation
        LoadTorque  = (10237.5)*(LoadTorque)+2047.5;    // convert Load Torque to bit representation
    ...
    void InitCpuTimer(void)
    {
        ...
        CpuTimer0Regs.TCR.bit.TIE = 1;   // enable CPU timer 0 interrupt
        CpuTimer0Regs.TIM.all = 0xF4240;
    ...
    void SetupADCEpwm(void) // Epwm sets trigger for ADCA and ADCC
    {
    ...
    void SetupADCTimer(void)
    {
        ...
        AdcbRegs.ADCSOC0CTL.bit.TRIGSEL = 1;        // Trigger on CPU Timer 0
        AdcbRegs.ADCINTSEL1N2.bit.INT1SEL = 0;      // End of SOC0 will set INT1 flag
        AdcbRegs.ADCINTSEL1N2.bit.INT1E = 1;        // Enable INT1 flag
        AdcbRegs.ADCINTFLGCLR.bit.ADCINT1 = 1;      // Make sure INT1 flag is cleared

        // Setup ADC-D3
        AdcdRegs.ADCSOC0CTL.bit.CHSEL = 3;          // SOC0 will convert pin C3 (HSEC Pin 33)
        AdcdRegs.ADCSOC0CTL.bit.ACQPS = 14;         // Sample window is 100 SYSCLK cycles
        AdcdRegs.ADCSOC0CTL.bit.TRIGSEL = 1;        // Trigger on CPU Timer 0
        EDIS;
    }
    ...
    // get and update input variables changed in GUI
    interrupt void adcb1_isr(void)
    {
        LoadTorque  = AdcbResultRegs.ADCRESULT0;   // update LoadTorque
        DutyCycle   = AdcdResultRegs.ADCRESULT0;    // update DutyCycle
    }
    ...
    interrupt void adca1_isr(void)
    {
        ...
        if (trigger != 0)
        {
            ...
            AdcaResults[resultsIndex] = 0.293 * AdcaResultRegs.ADCRESULT0 - 600;        // add ADC_A results to memory, first scaling to motor speed range
            AdccResults[resultsIndex++] = 0.00122 * AdccResultRegs.ADCRESULT0 - 2.5;    // add ADC_C results to memory, first scaling to armature current range
