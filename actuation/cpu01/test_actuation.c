...
// Function Prototypes
void PI_Loop(void);                 // Function for creating a PI Loop
...
float32 ideal_Speed = 1800;  // Simulated speed of motors on test bed with a threshold of 0-1800
...
float32 delta       = 60;   // Change in time for each interval (60Hz) for the cycles of data gathering
...
void main(void)
{
    ...
    do {
        ...
        PI_Loop(); // Call the PI Loop Function
    } while(1);
}
...
void PI_Loop(void)
{
    ...
    mm_Speed = AdcaResults[resultsIndex];  // Where 'AdcaResults[resultsIndex]' is going to be our newly defined mm_Speed in the most up to date code HSEC 15
    e = abs(ideal_Speed - mm_Speed);            // Current iteration error value calculation
